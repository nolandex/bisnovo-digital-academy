-- Create courses table (renaming from Bisnovo with proper case)
ALTER TABLE public."Bisnovo" RENAME TO courses;

-- Add new columns for course data
ALTER TABLE public.courses 
ADD COLUMN level TEXT DEFAULT 'Easy' CHECK (level IN ('Easy', 'Medium', 'Hard')),
ADD COLUMN rating DECIMAL(2,1) DEFAULT 4.5 CHECK (rating >= 1.0 AND rating <= 5.0),
ADD COLUMN students INTEGER DEFAULT 0,
ADD COLUMN modules INTEGER DEFAULT 3,
ADD COLUMN lessons INTEGER DEFAULT 12,
...
-- Clear existing data and insert sample course data
